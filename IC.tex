...
\begin{document} 
	...
	\begin{definicao}
		Tomemos (X, f) como um sistema dinâmico, onde X é o conjunto de todas as possibilidades de estado do sistema e f a função que regra o desenvolvimento desse conjunto X.
	\end{definicao}
	
	Tomando uma condição inicial $x_0 \in \mathbb{X}$, temos que a função f é dada por $x_t = kx_t-1$, ou seja, ($x_0, f(x_0), f(f(x_0), f(f(f(x_0))), ...$). Essa função também pode ser dada por outra notação ($x_0, f(x_0), f^2(x_0), f^3(x_0), ..., f^n(x_0), ...$) que é comumente mais utilizada, onde $n$ indica a quantidade de iteradas em f.
	
	\begin{exemplo}
		Um sistema dinâmico do tipo mais simples é uma função do tipo $f(x) = kx, k \in \mathbb{R}$.
	\end{exemplo}
	
	No entanto, esse é um tipo de função que restringe o sistema a três possibilidades:
	\begin{enumerate}
	...
	Assim sendo, um sistema dinâmico mais robusto que contempla um crescimento/decrescimento com mais variáveis e regras é dado por $f(x) = kx(1 - L)$, onde $k \in \mathbb{R}$ e $L$ é o limite populacional suportado pelo sistema. Nesse caso, trabalhamos L como porcentagem. Esse formato de sistema dinâmico é denominado família logística.
	...
	\end{definicao}
	\begin{definicao}
		Seja $n \in \mathbb{N}$ e $p$ um ponto periódico de período n. Chamamos um ponto $x_0$ de futuramente assintótico a $p$ se $$\lim_{i \longrightarrow \infty} f^{in}(x_0) = p$$
		Caso p não seja periódico, definimos um ponto $x_0$ como futuramente assintótico a $p$ se $$|f^i(x_0) - f^i(p)| \longrightarrow 0, i \longrightarrow \infty$$
		Esses pontos futuramente assintóticos compõem o que chamamos de conjunto estável do ponto $p$, denotado por $W^s(p)$. Caso a função $f$ admita inversa, chamamos de conjunto instável (pontos antigamente assintóticos) do ponto $p$, denotado por $W^u(p)$, aqueles pontos que satisfazem essas condições, mas com $i \longrightarrow -\infty$.
	\end{definicao}
	\begin{exemplo}
