...
\begin{document}
	...
	O objetivo deste capítulo é trazer exemplos de sistemas dinâmicos discretos para mostrar como eles ocorrem. Primeiramente, o termo "discreto" se refere à forma como o tempo é tratado: ele avança em passos contáveis $(n=0,1,2,3,...)$. Dado um valor inicial $x$, as iteradas da função exponencial, isto é,
	$$x, e^x, e^{e^x}, e^{e^{e^{x}}}, ...$$
	...
	\begin{exemplo}
		Vamos assumir que $L = 1$ seja o valor limite. Neste caso, não estamos falando agora sobre populações inteiras, mas sim sobre uma porcentagem da população. $P_n$ representa a porcentagem da população limite na geração $n$. Assume-se, então, que a população satisfaça a seguinte equação de diferenças:
		$$P_{n+1} = kP_n(1 - P_n),$$
