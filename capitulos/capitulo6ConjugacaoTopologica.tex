...
\begin{document}
	...
	\begin{prova}
		\begin{itemize}
			...
			Tomemos $x \in \Lambda$ e supomos que $S(x) = s_0s_1s_2 \dots$. Seja $\epsilon > 0$ e $n$ tal que $\frac{1}{2^n} < \epsilon$. Considere os subintervalos fechados $I_{t_0t_1\dots t_n}$ definidos acima para todas as combinações possíveis $t_0t_1 \dots t_n$. 
			
			Estes subintervalos são todos disjuntos, e $\Lambda$ está contido na união deles. Existem $2^{n+1}$ tais subintervalos, e $I_{s_0s_1\dots s_n}$ é um deles. Portanto, podemos escolher $\delta$ tal que $|x - y| < \delta$ e $y \in \Lambda$ implique que $y \in I_{s_0s_1\dots s_n}$. 
			
			Assim, $S(y)$ coincide com $S(x)$ nos primeiros $n + 1$ termos. Portanto, pelo Teorema da Proximidade:
			$$ d[S(x), S(y)] < \frac{1}{2^n} < \epsilon $$
			
			Isto prova a continuidade de $S$.
