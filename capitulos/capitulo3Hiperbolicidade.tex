...
\begin{document}
	...
	\begin{exemplo}
		Cada um dos mapas na Figura \ref{tresRetratosDeFases} satisfaz $f(0) = 0$ e $f'(0)=1$, mas cada um possui retrato de fase diferente próximo de $0$. em a), o mapa $f(x)=x+x^3$ tem um ponto repulsor em $0$. Em b), o mapa $f(x)=x-x^3$ tem um ponto fixo atrator em $0$. Já em c), o mapa $f(x)=x+x^2$ possui um ponto repulsor à direita mas atrator à esquerda. Um ponto fixo $p$ com $|f'(p)|=1$ é chamado de neutro ou indiferente.
			...
			\begin{figure}[H]
				...
				\end{tikzpicture}
				\caption{Retrato de fase de a)$f(x)=x+x^3$, b) $f(x)=x-x^3$, c)$f(x)=x+x^2$.}
				\label{tresRetratosDeFases}
			\end{figure}
	\end{exemplo}
	
	\section{Uma ideia sobre Bifurcação}

	Consideremos o parâmetro $\mu$ na função $f_\mu(x,\mu)=x^2+\mu$. Note que a existência de pontos fixos reais na função depende do parâmetro. Existe um determinado valor para $\mu$ tal que o gráfico da função não intersecta mais a reta $y = x$, fazendo com que pontos fixos deixem de existir. Portanto, mesmo uma pequena variação de $\mu$ pode mudar totalmente a estrutura de pontos fixos do sistema.
	\begin{figure}[H]
		\centering
	...
	\begin{exemplo}\label{primeiroExemploBifurcacao}
		\textbf{(Família Quadrática)} Tomemos o sistema dinâmico $(\mathbb{R}, f)$, onde $f_\mu(x,\mu)=x+\mu$ com sua derivada dada por $f_\mu'(x)=2x$. Logo, os pontos fixos da função são aqueles que satisfazem $x^2+\mu=x$.  Desse modo, temos que esses pontos são
		...
		\begin{itemize}
			...
			A partir do mapeamento do ponto fixo, é possível definir seu conjunto estável, dado pelos valores de $x$ que convergem para $\dfrac{1}{2}$, como mostrado na Figura \ref{retratoDeFaseEstavel}. Também é possível analisar o conjunto instável, onde essa análise se dá na função inversa.
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.4\textwidth]{imagens/desenho_conjunto_estavel_1-4.jpg}
				\caption{Análise de estabilidade do ponto fixo $x = \dfrac{1}{2}$}
				\label{retratoDeFaseEstavel}
			\end{figure}
			Assim, é possível concluir que o conjunto estável de $\dfrac{1}{2}$, ou seja, aqueles pontos em que $f^i(x) \rightarrow \dfrac{1}{2}, i \rightarrow \infty$  é dado por $W^s(\frac{1}{2}) = \{x \in \mathbb{R} : -\frac{1}{2} \leq x \leq \frac{1}{2}\}$. Isso significa que qualquer ponto fora do intervalo tem órbita convergindo para $\infty$.
			
			Já o conjunto instável, sob análise da função inversa, é facilmente descrito pelo seu gráfico usando o mesmo processo de retrato de fase na Figura \ref{retratoDeFaseInstavel}.
			
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.5\textwidth]{imagens/desenho_conjunto_instavel_1-4.png}
				\caption{Análise de instabilidade do ponto fixo $x = \dfrac{1}{2}$}
				\label{retratoDeFaseInstavel}
			\end{figure}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=0.6\textwidth]{imagens/desenho_conjunto_estavel_dois_pontos.png}
